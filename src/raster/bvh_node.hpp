...
inline bool bvh_node::bounding_box(const float t0, const float t1,
                                   aabb &b) const {
    (void) t0;
    (void) t1;
    b = box;
...
inline int box_x_compare(const void *a, const void *b) {
    ...
    if (!ah->bounding_box(0, 0, box_left) or
        !bh->bounding_box(0, 0, box_right)) {
        cerr << "no bounding box in bvh_node" << endl;
    }
...
inline bvh_node::bvh_node(hitable **l, int n, float time0, float time1) {
    ...
    aabb box_left, box_right;
    if (!left->bounding_box(time0, time1, box_left) or
...
// sphere -> bounding box (aabb) -> bvh_node(hold a box)
